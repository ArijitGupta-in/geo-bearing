# Contributing to @arijitgupta/geo-bearing
...
```bash
git clone https://github.com/ArijitGupta-in/geo-bearing.git
cd geo-bearing
npm install
```
...
| `src/bearingBetween.ts` | Initial bearing calculation |
...
- Validate all inputs at the public boundary. Throw `RangeError` for out-of-range or non-finite numeric values and include the bad value in the error message.
...
- Avoid `as any` in tests; use `as never` or `as const` when casting is genuinely necessary.
...
```plaintext
feat: add destination bearing helper
fix: validate non-finite coordinates
docs: clarify DMS coordinate support
...
Use the [issue tracker](https://github.com/ArijitGupta-in/geo-bearing/issues). Include a minimal reproduction and the version you are using.
